...
pub const Sim = struct {
    ...
    pub fn render(self: *const @This(), fb: []u8, w: u32, h: u32) void {
        ...
        var i: usize = 0;
        while (i < n) : (i += 1) {
            // Color computed from kind (a 3-entry lookup) — NOT stored in a
            // cold array. Stage 3 stored color in a cold array and skipped the
            // cold write on respawn (to avoid a read-for-ownership miss), which
            // left color STALE after respawn (kind re-rolled, color didn't) →
            // the three streams looked mixed. Stages 4/5 fixed this by
            // computing color from kind in render; stage 6 does the same. The
            // audit predicted this: color is a pure function of kind (density
            // 0.036 — a 3-entry dictionary), so it shouldn't be per-particle
            // data at all.
