...
**Status:** Stage 6 of 9 — last landed C7/stage 6 (**the payoff stage**: `@Vector(4, f32)` — 128-bit NEON — over stage 3's per-component SoA streams. Each NEON `fma` retires 4× the math of a scalar `fma`. P7: SIMD is a reward for layout — stage 3 laid the SoA streams and paid the overhead (losing to stage 2 on this toolchain); stage 6 finally collects the throughput reward. **The first stage to beat stage 2 on time**: 0.846 vs 1.102 ns/p at 1M (1.30× faster), beating stage 2 at every N≥65K. GB/s eff jumped from stage 3's ~17 to ~34 — the compute ceiling lifted and the loop approached bandwidth-bound. Width sweep confirms W=4 (native NEON) is the minimum. The roofline is visible: speedup narrows from 1.78× (65K, cache-resident) to 1.06× (64M, bandwidth-bound). Builds on stage 3's clean SoA — stages 4/5's compaction/sort will be recomposed in stage 9's synthesis.)
...
- **Stage 6 — the P7 payoff, measured:** total cycles **dropped from 5.71M to
  ...
  roof). **Stage 6 finally beats stage 2 on time** — 0.846 vs 1.102 ns/p at 1M
